...
const spreadsheetPath = "ImplementationSpecification.xlsx";
const structuresPath = "Structures/";
const outputsPath = "Examples/";
const strictXSD = "XSDs/SIF_Message.xsd"
const ignoreTags = [ "SIF_Metadata", "SIF_ExtendedElements" ];
const knownIds = ["refId", "RefId"];
...
if (!fs.existsSync(spreadsheetPath)) {
  console.log('File not found:', spreadsheetPath);
  process.exit(1);
}
...
function createWorksheetFields(sheet) {
  ...
  for (let row = 1; row < rows; row++) {
    ...
    const uniqueID = sheet[uniqueIDCellAddress] ? sheet[uniqueIDCellAddress].v : undefined;
    ...
    if (uniqueID) {
      ...
      for (let col = 0; col < cols; col++) {
        ...
        const columnHeaderCellAddress = xlsx.utils.encode_cell({ r: 0, c: col });
        const columnHeader = sheet[columnHeaderCellAddress] ? sheet[columnHeaderCellAddress].v : undefined;
...
function copyXmlChildren(source, destination) {
  ...
  if(sourceKey != destinationKey) {
    console.log("Warning:  Source and destination mismatch, copying children anyway!")
  ...
  for(const attributeKey of attributeKeys) {
    destination[destinationKey].$[attributeKey] = source[sourceKey].$[attributeKey];
  }
  // So we copy elements.
...
function traverseXML(template, xmlObject, fields, parent = null, xpath = '', ipath = '', ignore = false) {
  if (typeof template === 'object') {
    ...
    for (const key in template) {
      ...
      traverseXML(template[key], xmlObject, fields, newParent, currentXPath, currentIPath, ignore);
...
function callJina(groupXPath, children, fields) {
    ...
    for(const childXPath of children) {
      ...
      if('@' != childKey[0]) {
        const child = createXmlElement(childKey, {}, generateValue(childXPath, fields));

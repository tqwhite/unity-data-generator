...
const moduleFunction =
	...
	({ openai, vectorDb }) => {
		...
		const getEmbeddableData = (
			...
			// Limit to just 3 records to prevent processing all 15k records
			const embeddableData = vectorDb
				.prepare(
					`SELECT * FROM ${sourceTableName} LIMIT 3;`,
				)
				.all();
			xLog.status(`Limited to 3 records from ${sourceTableName} (full table has ~15k records)`);

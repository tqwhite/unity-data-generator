...
const moduleFunction =
	...
	({ unused } = {}) => {
		...
		const execute = ({ count, addToDupeSet }) => {
			xLog.status(
				`duplicateWorkerSets starting with count=${count}, addToDupeSet=${addToDupeSet}`,
			);
			...
			const errorList = findInvalidJsonFiles(targetDir);
			...
			try {
				...
				const { startingNumber, templateSet } = analyzeExistingFiles(
					targetDir,
					addToDupeSet,
				);
				...
				xLog.result(
					`Successfully created ${count} duplicate worker sets starting from ${startingNumber}`,
				);
		...
		// ---------------------------------------------------------------------
		// findInvalidJsonFiles - Find files that do not parse

		function findInvalidJsonFiles(dirPath) {
			const invalidFiles = [];
		...
		const analyzeExistingFiles = (targetDir, addToDupeSet) => {
			...
			const compensationFiles = files.filter((f) =>
				/^worker_compensation_report_\d+\.json$/.test(f),
			);
			const hoursFiles = files.filter((f) =>
				/^worker_paid_hours_report_\d+\.json$/.test(f),
			);

			// Check for unnumbered duplicates
			const unnumberedDuplicates = files.filter(
				(f) =>
					f.startsWith('worker_') &&
					f.endsWith('.json') &&
					!/^worker_\d+\.json$/.test(f) &&
					f !== 'worker.json',
			);

			if (
				workerFiles.length > 0 ||
				compensationFiles.length > 0 ||
				hoursFiles.length > 0 ||
				unnumberedDuplicates.length > 0
			) {
				if (!addToDupeSet) {
					throw new Error(
						'Duplicate files found. Use -addToDupeSet flag to add to existing duplicate set.',
					);
				}

				// Find the highest numbered set
				if (workerFiles.length > 0) {
					const numbers = workerFiles.map((f) =>
						parseInt(f.match(/worker_(\d+)\.json/)[1]),
					);
		...
		const loadTemplateSet = (targetDir, number) => {
			...
			const compensationPath = path.join(
				targetDir,
				`worker_compensation_report_${number}.json`,
			);
			const hoursPath = path.join(
				targetDir,
				`worker_paid_hours_report_${number}.json`,
			);

			if (
				!fs.existsSync(workerPath) ||
				!fs.existsSync(compensationPath) ||
				!fs.existsSync(hoursPath)
			) {
		...
		const loadOriginalTemplateSet = (targetDir) => {
			...
			const compensationPath = path.join(
				targetDir,
				'worker_compensation_report.json',
			);
		...
		const findCompleteUnnumberedSet = (targetDir) => {
			...
			const files = fs.readdirSync(targetDir);
			const workerDupes = files.filter(
				(f) =>
					f.startsWith('worker_') && f.endsWith('.json') && f !== 'worker.json',
			);
			...
			if (workerDupes.length > 0) {
				...
				if (files.includes(compensationFile) && files.includes(hoursFile)) {
					return {
						worker: JSON.parse(
							fs.readFileSync(path.join(targetDir, workerDupes[0]), 'utf8'),
						),
						compensation: JSON.parse(
							fs.readFileSync(path.join(targetDir, compensationFile), 'utf8'),
						),
						hours: JSON.parse(
							fs.readFileSync(path.join(targetDir, hoursFile), 'utf8'),
						),
					};
				}
			}
		...
		const generateDuplicates = (
			...
			for (let i = 0; i < count; i++) {
				...
				const newCompensation = JSON.parse(
					JSON.stringify(templateSet.compensation),
				);
				...
				const originalFirstName = newWorker.worker.qtGetSurePath(
					'name.firstName',
					'Worker',
				);
				...
				fs.writeFileSync(
					path.join(
						targetDir,
						`worker_compensation_report_${currentNumber}.json`,
					),
				...
				fs.writeFileSync(
					path.join(
						targetDir,
						`worker_paid_hours_report_${currentNumber}.json`,
					),

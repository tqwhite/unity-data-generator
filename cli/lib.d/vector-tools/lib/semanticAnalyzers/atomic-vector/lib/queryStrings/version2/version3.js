...
`;
	...
	const convertAtomicFactsToEmbeddingStrings = (
		...
		if (originalDefinition && originalDefinition.trim()) {
			embeddingStrings.push(originalDefinition.trim());
		}
		...
		if (extractedData?.elements && Array.isArray(extractedData.elements)) {
			extractedData.elements.forEach((el) => {
				...
				if (Array.isArray(el.facts)) {
					el.facts.forEach((factObj) => {
						if (factObj?.text && factObj.text.trim()) {
							embeddingStrings.push(factObj.text.trim());
						}
					});
				}
				...
				['Subject', 'Category', 'Data_Meaning'].forEach((key) => {
					if (Array.isArray(el[key])) {
						el[key].forEach((val) => {
							if (val && val.trim()) {
								embeddingStrings.push(`${key}: ${val.trim()}`);
							}
						});
					}
				});
				...
				if (el.canonical) {
					...
					Object.entries(el.canonical).forEach(([k, v]) => {
						if (v && v !== 'unknown') {
							embeddingStrings.push(`${k}: ${v}`);
							parts.push(`${k}=${v}`);
						}
					});
					if (parts.length) {
						embeddingStrings.push(`canonical: ${parts.join('; ')}`);
					}
				}
				...
				if (el.evidence?.path_cues && Array.isArray(el.evidence.path_cues)) {
					el.evidence.path_cues.forEach((cue) => {
						if (cue && cue.trim()) {
							embeddingStrings.push(`path_cue: ${cue.trim()}`);
						}
					});
				}
			});
		...
		if (originalPath) {
			derivePathCues(originalPath).forEach((c) =>
				embeddingStrings.push(`path_cue: ${c}`),
			);
		}

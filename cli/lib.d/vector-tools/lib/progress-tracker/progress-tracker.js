...
const moduleFunction =
	...
	({ unused } = {}) => {
		...
		// ---------------------------------------------------------------------
		// validateAndExecutePurgeProgressTable - validates and executes progress table purge
		

		const validateAndExecutePurgeProgressTable = ({vectorDb, config}) => {
			...
			try {
				purgeProgressTable(vectorDb, config.dataProfile);
				xLog.status(`Ready to start fresh for ${config.dataProfile} profile`);
			} catch (error) {
				xLog.error(
					`Purge progress table failed: ${error.message} [${moduleName}]`,
				);
				process.exit(1);
			}
		};
		...
		const createBatch = (
			...
			const actualTableName =
				semanticMode === 'atomicVector'
					? `${config.vectorTableName}_atomic`
		...
		const getProcessedKeys = (vectorDb, batchId) => {
			...
			const tableName =
				batch.semantic_mode === 'atomicVector'
					? batch.target_table
					: batch.target_table;
		...
		const purgeProgressTable = (vectorDb, dataProfile) => {
			...
			xLog.status(
				`Purged ${result.changes} progress entries for ${dataProfile} profile`,
			);
		...
		const showProgress = (vectorDb) => {
			...
			batches.forEach((batch) => {
				...
				xLog.status(
					`  Profile: ${batch.data_profile} | Mode: ${batch.semantic_mode} | Status: ${batch.status}`,
				);
				xLog.status(
					`  Progress: ${batch.processed_records}/${batch.total_records} (${progress}%)`,
				);
				xLog.status(
					`  Started: ${batch.start_time} | Updated: ${batch.last_update_time}`,
				);
				xLog.status('');
			});
		};
		...
		return {
			...
			purgeProgressTable,
			showProgress,
			validateAndExecutePurgeProgressTable,
		};
	};

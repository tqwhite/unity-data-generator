...
const moduleFunction = function (args = {}) {
	...
	const determineOperationType = (commandLineParameters) => {
		...
		if (values.queryString) {
			...
			thoughtProcess =
				semanticMode === 'atomicVector'
					? 'Vector_Atomic_Thought_Process'
					: 'Vector_Simple_Thought_Process';
		...
		const routingDecision = featureFlags.shouldUseFramework(
			operation,
			commandLineParameters,
		);

		xLog.verbose(
			`${moduleName}: Feature flag decision: ${JSON.stringify(routingDecision)}`,
		);
	...
	const executeFrameworkRoute = async (
		operationInfo,
		initAtp,
		commandLineParameters,
		legacyOperations,
		...
		try {
			...
			const { thoughtProcessConversationList, thinkerParameters } =
				getConfig(thoughtProcess);

			if (!thoughtProcessConversationList) {
				throw new Error(
					`Thought process '${thoughtProcess}' is not in the configuration file`,
				);
			}

			// Map CLI parameters to framework format
			const mappedParameters = parameterMapper.mapCliParametersToFramework(
				commandLineParameters,
				operation,
			);
			...
			xLog.verbose(
				`Mapped parameters: ${JSON.stringify(mappedParameters, null, 2)}`,
			);
			...
			facilitators = parameterMapper.injectParametersIntoFacilitators(
				facilitators,
				mappedParameters,
			);
			...
			const validation = featureFlags.validateFrameworkResult(
				wisdom,
				operation,
			);
			if (!validation.valid) {
				throw new Error(
					`Framework result validation failed: ${validation.reason}`,
				);
			}

			// Extract original parameters for result formatting
			const originalParams = parameterMapper.extractParametersFromOriginalCall
				? parameterMapper.extractParametersFromOriginalCall(
						commandLineParameters,
					)
				: {
			...
			const mappedResult = parameterMapper.mapFrameworkResultToCli(
				wisdom,
				operation,
				originalParams,
			);
			...
			if (mappedResult && mappedResult._legacyFormatOutput) {
				xLog.status(mappedResult._legacyFormatOutput);
			} else if (mappedResult) {
				// Fallback: If no formatted output, show basic results
				xLog.status('Framework results (unformatted):');
				if (mappedResult.results && Array.isArray(mappedResult.results)) {
					mappedResult.results.forEach((result, index) => {
						...
						const refId =
							result.refId || result.record?.GlobalID || `result${index + 1}`;
						const definition =
							result.record?.Definition ||
							result.definition ||
							'No description';
						xLog.status(
							`${index + 1}. [score: ${score}] ${refId} ${definition}`,
						);
					});
				} else {
					xLog.status('No results found or results format unrecognized');
				}
			} else {
			...
			const endTime = Date.now();
			featureFlags.recordFrameworkMetrics(
				operation,
				startTime,
				endTime,
				false,
	...
	const executeLegacyRoute = async (
		operationInfo,
		legacyOperations,
		commandLineParameters,
	) => {
	...
	const routeOperation = async (args) => {
		...
		xLog.status(`Process logs at: ${xLog.getProcessFilesDirectory()}`);

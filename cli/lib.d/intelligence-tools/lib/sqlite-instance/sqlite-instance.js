...
const moduleFunction = function ({ unused }) {
		...
		return (inObj, options = {}, callback) => {
			...
			taskList.push((args, next) => {
				...
				const localCallback = (err, createStatement) => {
					// Convert different quote styles to brackets for compatibility
					if (createStatement && createStatement[0] && createStatement[0].sql) {
						const sql = createStatement[0].sql;
						// Split on first opening paren to separate table name from column definitions
						const parts = sql.split('(');
						if (parts.length >= 2) {
							const tableNamePart = parts[0];
							const columnsPart = '(' + parts.slice(1).join('(');
							// Convert column definitions only
							const convertedColumns = columnsPart
								// First, convert "quoted" columns to [bracketed]
								.replace(/"([^"]+)"/g, '[$1]')
								// Then, handle unquoted columns
								.replace(/(\n\s*|,\s*)([a-zA-Z_][a-zA-Z0-9_]*)\s+(TEXT|INTEGER|REAL|BLOB|PRIMARY)/g, '$1[$2] $3');
							createStatement[0].sql = tableNamePart + convertedColumns;
						}
					}
	...
	const initTable = (runStatement, options, callback) => {
		...
		// xLog.error(`HACKED: SQL statement in [${moduleName}]`); // Commented out for vector-tools2
	...
	const initDatabaseInstance = (dbFilePath, callback) => {
		...
		// Expose the raw database for parameterized queries (needed for vector operations)
		callback('', { getTable, jsonToTsv, accessPoints: {}, db });
	};

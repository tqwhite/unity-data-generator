		...
		const getResponse = (passThroughObject, options = {}) => {
			...
			return new Promise(async (resolve, reject) => {
				try {
					...
					xLog.progress(`[${moduleName}] Completed array processing in ${currentWisdom._conversationMetadata[conversationId].duration}ms`);
					
					// Debug: Check if metadata exists
					if (currentWisdom._conversationMetadata) {
						xLog.progress(`[${moduleName}] Conversation metadata contains ${Object.keys(currentWisdom._conversationMetadata).length} entries`);

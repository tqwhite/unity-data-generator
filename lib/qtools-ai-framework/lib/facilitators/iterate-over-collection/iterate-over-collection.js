			...
			try {
				...
				const facilitatorAccessor = {
					wisdomAccessor: (value) => {
						wisdomAccessor = value;
						// Phase B: Set execution mode when wisdom-bus is injected
						if (value && value.setExecutionMode) {
							xLog.verbose(`${moduleName}: Wisdom-bus injected, setting to iteration mode`);
							value.setExecutionMode('iteration');
						}
					},
				};
						...
						try {
							...
							const itemResponse = await itemConversation.getResponse({
								args: {
									...
									initialData: itemWisdom  // Phase C: Pass item wisdom as initialData
								},

 ...
 *    - For each item: itemWisdom = { ...latestWisdom, [itemKey]: currentItem }
 *    - Default itemKey is 'currentElement' (configurable)
 ...
 * -------------------------
 * - itemKey must match what downstream thinkers expect (usually 'currentElement')
	...
	const facilitator = ({ latestWisdom, args }) => {
		return new Promise(async (resolve, reject) => {
			try {
				...
				xLog.status(`${moduleName}: Completed processing ${successCount} items successfully, ${errorCount} errors in ${duration}ms`);

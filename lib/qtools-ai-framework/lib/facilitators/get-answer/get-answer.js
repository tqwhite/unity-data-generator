...
const moduleFunction = function ({
	...
	xLog.progress(`using conversation '${conversationName}' in [${moduleName}]`);
	

	let wisdomAccessor;
	const facilitatorAccessor = {
		wisdomAccessor: (value) => {
			facilitatorName: moduleName, (wisdomAccessor = value);
		},
	};

	const jinaConversation = jinaCore.conversationGenerator({
		conversationName,
		thinkerParameters, // Pass to conversation generator
		thoughtProcessName, // Pass to conversation generator
		facilitatorAccessor,

...
function getCallerInfo() {
	...
	// stackLines[1] is this function (getCallerInfo)
	// stackLines[2] is the *caller* of this function
	const callerLine = stackLines[stackLines.length - 1] || '';
	const match =
		callerLine.match(/at\s+(.*?)\s+\((.*)\)/) || callerLine.match(/at\s+(.*)/);
	return callerLine.replace(/\/Users\/tqwhite\/Documents\/webdev\/A4L\/unityObjectGenerator\/system\/code\/cli\/lib.d\/unity-data-generator/, '...').replace(/at accessSmartyPants /, '');
	return match ? match[1] : 'unknown';
}

// Example usage
// function exampleCaller() {
//   console.log('Called by:', getCallerInfo());
// }
// exampleCaller();
...
const moduleFunction = function ({ accessParms, model, promptList }) {
	...
	const getCompletion = (args, callback) => {
		...
		taskList.push(async (args, next) => {
			...
			if (!model.match(/^o/)) {
				parameters.temperature = temperatureFactor ? temperatureFactor : 0.0;
			}

			try {
				rawAiResponseObject = await openai.chat.completions.create(parameters);
			} catch (err) {
				xLog.error(
				...
				next(err, {
					wisdom: err.toString(),
					rawAiResponseObject: { isError: true, err, promptList },
				});
				return;
			}
		...
		xLog.progress(`${moduleName} called by ${getCallerInfo()}`);

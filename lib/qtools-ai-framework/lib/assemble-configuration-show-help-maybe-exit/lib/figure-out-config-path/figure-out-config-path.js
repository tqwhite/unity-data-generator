...
const moduleFunction = function ({ xxx } = {}) {
	...
	const getConfigPath = ({ fileString, configFileBaseName } = {}) => {
		...
		if (fileString && fs.existsSync(fileString)) {
			outString=fileString; //return user override config file path
		}

		if (fs.existsSync(qbookFilePath)) {
			outString=qbookFilePath; // this one is special for TQ
		}

		if (fs.existsSync(homeUserNameFilePath)) {
			outString=homeUserNameFilePath; // this one is any other developers
		}

		if (fs.existsSync(moduleNameFilePath)) {
			outString=moduleNameFilePath; // This is more specific for special (cli) situations
		}

		if (fs.existsSync(configFilePath)) {
			outString=configFilePath; // normal deployment has config data at top level of directory so other config's secrets are preserved
		}
		...
		xLog.progress(`No configuration file found at any of these paths:\n    ${filePaths.join('\n    ')}\n`);

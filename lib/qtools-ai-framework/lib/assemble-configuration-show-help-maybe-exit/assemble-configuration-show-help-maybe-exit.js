...
const defaultHelpText = require('./lib/help-text');
...
const moduleFunction = function ({
	...
	taskList.push((args, next) => {
		...
		xLog.progress(`Using config file: ${configFilePath}`);
		...
		if (help) {
			process.stderr.write(
				(helpText ? helpText : defaultHelpText)
					...
					.qtTemplateReplace(defaultHelpText)
			); //applies defaultHelpText.frameworkHelpInfo to <!frameworkHelpInfo!> in text
			process.exit();
			// 			next('skipRestOfPipe');
			// 			return;
		}
	...
	asynchronousPipePlus.pipeRunner(
		...
		(err, finalResult = {}) => {
			...
			if (err) {
				process.global.error = process.global.error
					? { ...process.global.error, [moduleName]: err }
					: { [moduleName]: err };
				xLog.error(process.global.error);
